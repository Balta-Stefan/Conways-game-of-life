...
void Canvas::readImage(std::string file)
{
    ...
    std::FILE* fp = std::fopen(filename, "rb"); /* b - binary mode */
    if (!fscanf(fp, "P5\n%d %d\n255\n", &numOfRealHorizontalCells, &numOfRealVerticalCells))
        return;
    ...
    unsigned char* image = new unsigned char[numOfRealHorizontalCells * numOfRealVerticalCells];
    fread(image, sizeof(unsigned char), numOfRealHorizontalCells * numOfRealVerticalCells, fp);
    fclose(fp);
    ...
    for(int i = 0; i < rowLength*numOfRealVerticalCells; i++)
    {
        unsigned char eightCells = 0;
        for(int j = 0; j < 8; j++) //8 contiguous cells are put into unsigned char
        {
            if(image[counter++] == 0)
                eightCells |= one << (7-j);
        }
...
void Canvas::writeImage(std::string file)
{
    if(file == "")
        return;
    ...
    char* image = new char[numOfRealVerticalCells * numOfRealHorizontalCells];
    int counter = 0;
    //unpack the world
    for(int i = 0; i < rowLength * numOfRealVerticalCells; i++)
    {
        unsigned char eightCells = world[i];
        for(int j = 0; j < 8; j++) //8 cells are stored in an unsigned char
        {
            char cellValue = 127;
            if(((eightCells >> (7-j)) & 0x01) == 1)
                cellValue = 0;
            image[counter++] = cellValue;
        }
    }

    fwrite(image, sizeof(char), numOfRealHorizontalCells * numOfRealVerticalCells, fp);
    fclose(fp);
    delete[] image;
}
...
void Canvas::wheelEvent(QWheelEvent *event)
{
    ...
    int cellX, cellY;
    int partX = (double)(((int)event->position().x()-offsetX) % cellWidth)/cellWidth * 100.0, partY = (double)(((int)event->position().y()-offsetY) % cellHeight)/cellHeight * 100.0;

    determineClickedCell((int)event->position().x(), (int)event->position().y(), cellX, cellY);
    ...
    int screenOffsetX = newPartXCoordinate - (int)event->position().x();
    int screenOffsetY = newPartYCoordinate - (int)event->position().y();
